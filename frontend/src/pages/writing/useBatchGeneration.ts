...
export function useBatchGeneration(args: {
  ...
  const startBatchGeneration = useCallback(async () => {
    if (!projectId) return;
    if (!preset) {
      toast.toastError("请先在 Prompts 页面保存 LLM 预设。");
      return;
    }
    ...
    try {
      ...
      toast.toastSuccess("批量生成任务已启动。", res.request_id);
    } catch (e) {
      const err = e as ApiError;
