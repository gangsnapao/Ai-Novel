...
def _kind_breakdown(rows: list[Any], *, done_statuses: set[str]) -> list[dict[str, Any]]:
    buckets: dict[str, dict[str, Any]] = {}

    for row in rows:
        kind = str(getattr(row, "kind", "") or "").strip() or "unknown"
        status = str(getattr(row, "status", "") or "").strip().lower()
        bucket = buckets.setdefault(kind, {"kind": kind, "total": 0, "queued": 0, "running": 0, "done": 0, "failed": 0})
        bucket["total"] += 1
        if status == "queued":
            bucket["queued"] += 1
        elif status == "running":
    ...
    return sorted(buckets.values(), key=lambda item: (-int(item["failed"]), -int(item["running"]), -int(item["total"]), str(item["kind"])))
...
def _task_metrics(rows: list[Any], *, done_statuses: set[str]) -> dict[str, Any]:
    ...
    return {
        ...
        "kind_breakdown": _kind_breakdown(rows, done_statuses=done_statuses),
    }
    ...
    imports = (
        db.execute(
            select(ProjectSourceDocument).where(
                ...
                ProjectSourceDocument.created_at >= start,
            )
        )
        .scalars()
        .all()
